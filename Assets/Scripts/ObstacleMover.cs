...
public class ObstacleMover : MonoBehaviour
{
    ...
    // Per-object randomisation
    private float movementOffset;
    private float rotationOffset;
    private float moveSpeedVariation;
    ...
    void Start()
    {
        ...
        movementOffset = Random.Range(0f, 10f);   // offset in movement timing
        rotationOffset = Random.Range(0f, 10f);   // offset in rotation timing

        moveSpeedVariation = moveSpeed * Random.Range(0.85f, 1.15f);       // ±15% variation
        rotationSpeedVariation = rotationSpeed * Random.Range(0.85f, 1.15f);
    }
    ...
    void HandleMovement()
    {
        if (loopMovement)
        {
            float pingPong = Mathf.PingPong((Time.time + movementOffset) * moveSpeedVariation, 1f);
        ...
        {
            float t = Mathf.Clamp01((Time.time + movementOffset) * moveSpeedVariation / moveDistance);
    ...
    void HandleRotation()
    {
        ...
        {
            transform.Rotate(rotationAxis.normalized * rotationSpeedVariation * Time.deltaTime, Space.Self);
        }
        else // BackAndForth
        {
            if (loopRotation)
            {
                float pingPong = Mathf.PingPong((Time.time + rotationOffset) * rotationSpeedVariation, 1f);
            ...
            {
                float t = Mathf.Clamp01((Time.time + rotationOffset) * rotationSpeedVariation / rotationExtent);
